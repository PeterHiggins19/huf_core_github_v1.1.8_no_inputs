...
## 2) Paste over: `docs/huf_reference.md` (canonical HUF Reference, v1.3.0)
...
## What this page is
A skeptic-facing reference: expected academic critiques and precise, conservative answers. Use it to keep scope tight and claims credible.

!!! note "Ethics disclosure (short)"
    Drafted with AI assistance as an editing tool and reviewed/curated by the author. Formal claims are stated conservatively and should be reproducible via repository code where applicable.

## Why it matters
HUF can feel counter-intuitive: discarding “loss” (tail mass) can make what remains look stronger after re-normalization. This reference gives readers conceptual hooks to keep thinking instead of switching off.

## Key intuition: why loss can make retained stronger
If you keep only a retained subset and then re-normalize, every retained weight is scaled by the same factor `1 / retained_mass`. Example:
...
- Run a known example end-to-end (vector DB coherence is a good start).
- Confirm invariance: multiply all input scores by `10` and verify regime-level outputs are unchanged.
- Inspect `artifact_3_trace_report.jsonl` to ensure discards are explainable and consistent.
- Confirm the `artifact_4_error_budget.json` accounts for all removed mass.

## Next steps
- If you introduce new analogies, keep them labeled **interpretive** unless you also provide a formal construction.
- Keep version tags in titles (e.g., v1.3.0) so partners can cite stable documents.
